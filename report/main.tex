...
\subsection{Default training}
Here the network will be trained with default training parameters, i.e. each sample in the training input will be weighted equally in the loss function.
...
When the size of the dataset is not too large, there is a risk of overfitting. One way to battle this is data augmentation, where the dataset is in some way extended. For this test the images will be mirrored along the along the sagittal axis (right side of the brain will become the left), to make the training data twice as large.
