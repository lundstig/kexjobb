...
\textcite{islam2018early} studied a variation Alzheimer's disease classification, where there are four different classes based on CDR scores, instead of just two. This was done using a variant of a CNN trained on the OASIS dataset~\cite{oasis}. Their CNN uses uses a densely connected architecture that seems identical to DenseNet~\cite{huang2017densely}. While their model showed acceptable performance for identifying non-demented patients, its performance on classifying demented patients was significantly worse, which they speculate could be because of the limited number of samples~\cite{islam2018early} in OASIS.
